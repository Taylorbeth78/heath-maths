...
export default function DoNow(){
  ...
  function doPrint(){
    ...
    w.document.write('<html><head><title>Do Now</title><style>@page{size:A4 landscape;margin:12px}*{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}body{padding:14px;color:#1a1a1a}.header{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #2d6a2d;padding-bottom:8px;margin-bottom:10px}h1{font-size:17px;color:#2d6a2d;font-weight:700}.vocab{background:#fdf6e3;border:2px solid #c9a227;border-radius:8px;padding:10px 14px;margin-bottom:10px;display:flex;gap:10px}.vbody{flex:1}.vword{font-size:16px;font-weight:700;margin-bottom:2px}.vdef{font-size:12px;color:#333;margin-bottom:2px}.vex{font-size:11px;color:#666;font-style:italic}.vtask{margin-top:5px;font-size:11px;font-weight:600;color:#a07d10;border-top:1px solid #e8d080;padding-top:5px}.grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px}.card{border-radius:8px;overflow:hidden;border:1px solid #ddd}.card-hd{padding:7px 12px;font-size:10px;font-weight:700;color:white;text-transform:uppercase;display:flex;justify-content:space-between}.card-body{padding:12px;min-height:80px}.card-q{font-size:14px;font-weight:500;line-height:1.4}.card-ans{margin-top:8px;padding:5px 8px;border-radius:4px;font-size:11px;font-weight:600}.space{border-bottom:1px solid #ccc;margin-top:28px}.ch{border:2px solid #c9a227;border-radius:8px;overflow:hidden;margin-bottom:10px}.chl{background:#c9a227;padding:7px 12px;font-size:10px;font-weight:700;color:white;text-transform:uppercase}.ch-body{padding:12px}.footer{text-align:center;font-size:10px;color:#999;border-top:1px solid #eee;padding-top:6px;margin-top:8px}</style></head><body>'+(()=>{
const cats={'Number':'#2563eb','Algebra':'#7c3aed','Geometry':'#059669','Statistics':'#d97706','Functional':'#dc2626'};
const tc={'Place value & rounding':'Number','Fractions':'Number','Decimals':'Number','Percentages':'Number','Powers & roots':'Number','Standard form':'Number','Ratio & proportion':'Number','Compound interest':'Number','Simplifying expressions':'Algebra','Expanding brackets':'Algebra','Factorising':'Algebra','Solving linear equations':'Algebra','Solving quadratics':'Algebra','Simultaneous equations':'Algebra','nth term sequences':'Algebra','Straight line graphs':'Algebra','Rearranging formulae':'Algebra','Area of 2D shapes':'Geometry','Perimeter':'Geometry','Volume & surface area':'Geometry','Angles in polygons':'Geometry','Pythagoras theorem':'Geometry','Trigonometry':'Geometry','Circle theorems':'Geometry','Transformations':'Geometry','Bearings':'Geometry','Mean, median, mode':'Statistics','Bar charts':'Statistics','Scatter graphs':'Statistics','Probability':'Statistics','Cumulative frequency':'Statistics','Box plots':'Statistics','Venn diagrams':'Statistics','Money & budgeting':'Functional','Time calculations':'Functional','Measurement conversions':'Functional','Reading charts':'Functional','Percentages in context':'Functional','Area & perimeter in context':'Functional','Averages in context':'Functional','Probability in context':'Functional','Scale & maps':'Functional','Data interpretation':'Functional'};
const el=document.querySelector('[data-print]');
if(!el)return '';
const data=JSON.parse(el.dataset.print);
const color=t=>cats[tc[t]]||'#2d6a2d';
const stars=d=>'&#9733;'.repeat(d)+'&#9734;'.repeat(6-d);
const litTask=data.litType==='definition'?'Write the definition in your own words: ________________________________________':'Use this word in a sentence: ________________________________________________';
const showAns=data.showAns;
const cards=data.questions.map((q,i)=>'<div class="card"><div class="card-hd" style="background:'+color(q.topic)+'"><span>Q'+(i+1)+' - '+q.topic+'</span><span>'+stars(q.difficulty)+'</span></div><div class="card-body"><div class="card-q">'+q.q+'</div>'+(showAns?'<div class="card-ans" style="background:#dcfce7;color:#166534">'+q.a+'</div>':'<div class="space"></div>')+'</div></div>').join('');
return '<div class="header"><div><h1>Heath School - Maths Do Now</h1><div style="font-size:12px;color:#555">'+data.yg+'</div></div><div style="text-align:right;font-size:11px;line-height:1.8">Date: '+data.date+'<br>Name: _______________________<br>Class: _______________________</div></div><div class="vocab"><div style="font-size:26px">'+data.vocab.e+'</div><div class="vbody"><div style="font-size:10px;font-weight:700;color:#a07d10;text-transform:uppercase;margin-bottom:4px">Key Word - Literacy Focus</div><div class="vword">'+data.vocab.word+'</div><div class="vdef">'+data.vocab.def+'</div><div class="vex">'+data.vocab.ex+'</div><div class="vtask">'+litTask+'</div></div></div><div class="grid">'+cards+'</div><div class="ch"><div class="chl">Challenge</div><div class="ch-body"><div class="card-q">'+data.challenge.q+'</div>'+(showAns?'<div class="card-ans" style="background:#fef9c3;color:#854d0e">'+data.challenge.a+'</div>':'<div class="space"></div>')+'</div></div><div class="footer">Heath School Maths Hub - '+data.yg+' - '+data.date+'</div>';
})()+'</body></html>')
